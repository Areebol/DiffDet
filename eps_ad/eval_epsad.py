...
def detection_test_ensattack(args, config, model, loader):
    ...
    if config.data.dataset == "CIFAR10":
        args.num_classes = 10
        mean = (
            torch.from_numpy(np.array([x / 255 for x in [125.3, 123.0, 113.9]]))
            .view(1, 3, 1, 1)
            .cuda()
            .float()
        )
        std = (
            torch.from_numpy(np.array([x / 255 for x in [63.0, 62.1, 66.7]]))
            .view(1, 3, 1, 1)
            .cuda()
            .float()
        )
        ...
        mean = (
            torch.from_numpy(np.array([x / 255 for x in [129.3, 124.1, 112.4]]))
            .view(1, 3, 1, 1)
            .cuda()
            .float()
        )
        std = (
            torch.from_numpy(np.array([x / 255 for x in [68.2, 65.4, 70.4]]))
            .view(1, 3, 1, 1)
            .cuda()
            .float()
        )
    ...
    sde = sde_lib.VPSDE(
        beta_min=rev_vpsde.beta_0, beta_max=rev_vpsde.beta_1, N=rev_vpsde.N
    )
    ...
    score_adv_lists = []
    assert not (
        args.detection_ensattack_norm_flag == True
        and args.single_vector_norm_flag == True
    )
    ...
    for attack_method in attack_methods:
        ...
        for i, (x, y) in enumerate(loader):
            ...
            if not args.clean_score_flag:
                if attack_method == "MM_Attack" or attack_method == "AA_Attack":
                    ...
                    x = x[correct_batch]
                    y = y[correct_batch]
                x_adv, top1, top5 = ens_attack(
                    x, y, classifier, mean, std, args, attack_method
                )
            else:
                x_adv = x
                top1, top5 = 1, 1
            ...
            with torch.no_grad():
                for value in range(1, args.diffuse_t + 1):  # t 个 EPS 均值
                    # if value>3:
                    # 	break
                    t_valuve = value / 1000
                    ...
                    if args.perb_image:
                        ...
                        mean_x_adv, std_x_adv = sde.marginal_prob(
                            2 * x_adv - 1, curr_t_temp.expand(x_adv.shape[0])
                        )  # x_adv：原始样本，curr_t_temp：具体时间步
                        perturbed_data = mean_x_adv + std_x_adv[:, None, None, None] * z
                        score = score_fn(
                            perturbed_data, curr_t_temp.expand(x_adv.shape[0])
                        )
                    else:
                        score = score_fn(
                            2 * x_adv - 1, curr_t_temp.expand(x_adv.shape[0])
                    ...
                    if args.domain == "imagenet":
                        ...
                        assert (
                            x_adv.shape == score.shape
                        ), f"{x_adv.shape}, {score.shape}"
                    if args.detection_ensattack_norm_flag:
                        score_adv_list.append(
                            score.detach()
                            .view(x_adv.shape[0], -1)
                            .norm(dim=-1)
                            .unsqueeze(0)
                        )
                ...
                if args.generate_1w_flag:
                    ...
                    if args.detection_ensattack_norm_flag:
                        ...
                        for n in range(score_sum.size(0)):
                            np.save(
                                path_1w + "/{}.npy".format(j + n + 1),
                                ((score_sum[n]) / value).cpu().numpy(),
                            )
                    else:
                        exit(0)
                    ...
                    if args.detection_ensattack_norm_flag:
                        ...
                        score_adv_lists.append(
                            torch.cat(score_adv_list, dim=0)
                            .view(len(score_adv_list), *x_adv.shape)
                            .cpu()
                        )
        ...
        if not args.generate_1w_flag:
            print(
                f"attack_method: {attack_method}, robust accuracy: top1:{top1_counter/num_samples}--top5:{top5_counter/num_samples}"
            )
            print(f"attack and diffuison time: {time.time() - start_time}")
            score_tensor = (
                torch.cat(score_adv_lists, dim=1)
                if not args.single_vector_norm_flag
                else torch.cat(score_adv_lists, dim=0)
            )
            ...
            if not args.clean_score_flag:
                if not args.single_vector_norm_flag:
                    np.save(
                        f"{args.detection_datapath}/scores_adv_{attack_method}_{args.epsilon}_{args.num_steps}{isnorm}{value}{isperb_image}{data_size}.npy",
                        score_tensor.data.cpu().numpy(),
                    )
                else:
                    np.save(
                        f"{args.detection_datapath}/scores_adv_{attack_method}_{args.epsilon}_{args.num_steps}single_vector_norm{value}{isperb_image}{data_size}.npy",
                        score_tensor.data.cpu().numpy(),
                    )
                ...
                if not args.single_vector_norm_flag:
                    np.save(
                        f"{args.detection_datapath}/scores_clean{isnorm}{value}{isperb_image}{data_size}.npy",
                        score_tensor.data.cpu().numpy(),
                    )
                else:
                    np.save(
                        f"{args.detection_datapath}/scores_clean{isnorm}single_vector_norm{value}{isperb_image}{data_size}.npy",
                        score_tensor.data.cpu().numpy(),
                    )

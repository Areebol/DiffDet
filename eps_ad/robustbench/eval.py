...
import torch
import random
from eps_ad.autoattack import AutoAttack
from torch import nn
from tqdm import tqdm

from eps_ad.robustbench.data import (
    CORRUPTIONS,
    get_preprocessing,
    load_clean_dataset,
...
from eps_ad.robustbench.utils import clean_accuracy, load_model, parse_args, update_json
...
def benchmark(
    ...
    if threat_model_ in {ThreatModel.Linf, ThreatModel.L2}:
        ...
        x_adv = adversary.run_standard_evaluation(
            clean_x_test, clean_y_test, bs=batch_size
        )
        ...
        adv_accuracy = corruptions_evaluation(
            batch_size,
            data_dir,
            dataset_,
            device,
...
def corruptions_evaluation(
    ...
    existing_results_path = (
        Path("model_info") / dataset.value / "corruptions" / "unaggregated_results.csv"
    )

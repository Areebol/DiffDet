...
from eps_ad.robustbench.model_zoo.architectures.dm_wide_resnet import (
    CIFAR100_MEAN,
    CIFAR100_STD,
    DMWideResNet,
    Swish,
...
from eps_ad.robustbench.model_zoo.architectures.resnet import (
    PreActBlock,
    PreActResNet,
    PreActBlockV2,
    ResNet,
...
from eps_ad.robustbench.model_zoo.architectures.resnext import (
    CifarResNeXt,
    ResNeXtBottleneck,
)
from eps_ad.robustbench.model_zoo.architectures.wide_resnet import WideResNet
from eps_ad.robustbench.model_zoo.enums import ThreatModel
from eps_ad.robustbench.model_zoo.architectures.CARD_resnet import (
    LRR_ResNet,
    WidePreActResNet,
)
...
class Chen2020EfficientNet(WideResNet):
    def __init__(self, depth=34, widen_factor=10):
        super().__init__(
            depth=depth, widen_factor=widen_factor, sub_block1=True, num_classes=100
        )
        self.register_buffer(
            "mu", torch.tensor([0.5071, 0.4867, 0.4408]).view(1, 3, 1, 1)
        )
        self.register_buffer(
            "sigma", torch.tensor([0.2675, 0.2565, 0.2761]).view(1, 3, 1, 1)
        )
...
class Wu2020AdversarialNet(WideResNet):
    def __init__(self, depth=34, widen_factor=10):
        super().__init__(
            depth=depth, widen_factor=widen_factor, sub_block1=False, num_classes=100
        )
        self.register_buffer(
            "mu",
            torch.tensor(
...
class Rice2020OverfittingNet(PreActResNet):
    def __init__(self):
        super(Rice2020OverfittingNet, self).__init__(
            PreActBlock,
            [2, 2, 2, 2],
            num_classes=100,
            bn_before_fc=True,
...
class Hendrycks2019UsingNet(WideResNet):
    def __init__(self, depth=28, widen_factor=10):
        super(Hendrycks2019UsingNet, self).__init__(
            depth=depth, widen_factor=widen_factor, num_classes=100, sub_block1=False
        )
...
class Diffenderfer2021CARD(LRR_ResNet):
    def __init__(self, width=128, num_classes=100):
        ...
        self.register_buffer(
            "mu", torch.tensor([0.5071, 0.4865, 0.4409]).view(1, 3, 1, 1)
        )
        self.register_buffer(
            "sigma", torch.tensor([0.2673, 0.2564, 0.2762]).view(1, 3, 1, 1)
        )
...
linf = OrderedDict(
    [
        ...
        (
            ...
            {
                "model": lambda: WideResNet(num_classes=100, depth=34, sub_block1=True),
...
cifar_100_models = OrderedDict(
    [(ThreatModel.Linf, linf), (ThreatModel.corruptions, common_corruptions)]
)

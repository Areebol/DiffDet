...
from eps_ad.robustbench.model_zoo import model_dicts as all_models
from eps_ad.robustbench.model_zoo.enums import BenchmarkDataset, ThreatModel
...
def load_model(
    ...
    if not isinstance(models[model_name]["gdrive_id"], list):
        ...
        if (
            "Kireev2021Effectiveness" in model_name
            or model_name == "Andriushchenko2020Understanding"
        ):
            checkpoint = checkpoint[
                "last"
            ]  # we take the last model (choices: 'last', 'best')
        try:
    ...
    # If we have an ensemble of models (e.g., Chen2020Adversarial, Diffenderfer2021Winning_LRR_CARD_Deck)
        ...
        for i, gid in enumerate(models[model_name]["gdrive_id"]):
            ...
            checkpoint = torch.load(
                "{}_m{}.pt".format(model_path, i), map_location=torch.device("cpu")
            )
            try:
            ...
            model.models[i] = _safe_load_state_dict(
                model.models[i], model_name, state_dict, dataset_
            )
            model.models[i].eval()
...
def _safe_load_state_dict(
    ...
    try:
        ...
        if (
            model_name in known_failing_models or dataset_ == BenchmarkDataset.imagenet
...
def list_available_models(
    ...
    for model_name in models:
        ...
        json_dict["venue"] = (
            "Unpublished" if json_dict["venue"] == "" else json_dict["venue"]
        )
    ...
    for i, json_dict in enumerate(json_dicts):
        if json_dict["model_name"] == "Chen2020Adversarial":
            json_dict["architecture"] = (
                json_dict["architecture"] + " <br/> (3x ensemble)"
            )
        if json_dict["model_name"] != "Natural":
            print(
                "| <sub>**{}**</sub> | <sub><sup>**{}**</sup></sub> | <sub>*[{}]({})*</sub> | <sub>{:.2%}</sub> | <sub>{:.2%}</sub> | <sub>{}</sub> | <sub>{}</sub> |".format(
                    i + 1,
                    json_dict["model_name"],
                    json_dict["name"],
                    json_dict["link"],
                    json_dict["clean_acc"],
...
def get_leaderboard_bibtex(
    dataset: Union[str, BenchmarkDataset], threat_model: Union[str, ThreatModel]
):
    dataset_: BenchmarkDataset = BenchmarkDataset(dataset)
    threat_model_: ThreatModel = ThreatModel(threat_model)
...
def get_leaderboard_latex(
    ...
    for json_path in jsons_dir.glob("*.json"):
        ...
        str_curr = (
            "\\citet{{{}}}".format(model_name)
            if not model_name in ["Standard", "Standard_R50"]
...
def update_json(
    ...
    json_path = (
        Path("model_info") / dataset.value / threat_model.value / f"{model_name}.json"
    )
    ...
    model_info = ModelInfo(
        dataset=dataset.value, eps=eps, clean_acc=accuracy, **acc_field_kwarg
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--n_ex", type=int, default=100, help="number of examples to evaluate on"
    )
    parser.add_argument(
        "--batch_size", type=int, default=500, help="batch size for evaluation"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    parser.add_argument(
        "--device", type=str, default="cuda:0", help="device to use for computations"
    )

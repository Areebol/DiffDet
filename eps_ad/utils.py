...
from eps_ad.robustbench import load_model
import eps_ad.data as data
...
def load_detection_data(args, adv_batch_size, generate_1w_flag=False):
    if "imagenet" in args.domain:
        ...
        val_data = data.imagent_dataset_sub(
            data_dir,
            transform=transform,
            num_sub=args.num_sub,
            data_seed=args.data_seed,
            train=generate_1w_flag,
        )
        loader = DataLoader(
            val_data,
            batch_size=adv_batch_size,
...
def load_detection_train_test(args, adv_batch_size):
    if "imagenet" in args.domain:
        data_dir = args.datapath
        transform = data.get_transform(args.domain, "imval", base_size=224)
        train_data = data.imagent_dataset_sub(
            data_dir,
            transform=transform,
            num_sub=args.num_sub_train,
            data_seed=args.data_seed,
            train=True,
        )
        ...
        data_dir = args.datapath
        transform = transforms.Compose([transforms.ToTensor()])
        train_data = data.cifar10_dataset_sub(
            data_dir,
            transform=transform,
            num_sub=args.num_sub_train,
            data_seed=args.data_seed,
...
def load_OOD_data(args, adv_batch_size):
    if "imagenet" in args.domain:
        ...
        data_dir = args.datapath
        transform = transforms.Compose([transforms.ToTensor()])
        val_data = data.cifar100_dataset_sub(
            data_dir,
            transform=transform,
            num_sub=args.num_sub,
            data_seed=args.data_seed,
